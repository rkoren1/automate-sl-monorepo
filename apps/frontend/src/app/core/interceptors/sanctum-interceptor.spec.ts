...
import { SANCTUM_PREFIX } from 'main/@core/bootstrap/sanctum.service';
...
describe('SanctumInterceptor', () => {
  ...
  const setBaseUrlAndSanctumPrefix = (
    baseUrl: string | null,
    sanctumPrefix: string | null
  ) => {
  ...
  it('should get csrf cookie with base url', () => {
    ...
    httpMock
      .expectOne('https://foo.bar/sanctum/csrf-cookie')
      .flush({ cookie: true });
    httpMock.expectOne('/auth/login').flush({ login: true });
    httpMock.expectOne('/me').flush({ me: true });
  });
  ...
  it('should get csrf cookie with base url and sanctum prefix', () => {
    ...
    httpMock
      .expectOne('https://foo.bar/foobar/csrf-cookie')
      .flush({ cookie: true });
    httpMock.expectOne('/auth/login').flush({ login: true });
    httpMock.expectOne('/me').flush({ me: true });
  });
